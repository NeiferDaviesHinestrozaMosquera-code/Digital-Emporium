...
export default function QuoteRequestForm({ availableServices, lang }: QuoteRequestFormProps) {
  const { toast } = useToast();
  const router = useRouter(); // Keep for potential future client-side navigation needs
  ...
  async function onSubmit(values: QuoteRequestFormValues) {
    ...
    try {
      ...
      if (result.success) {
        form.reset(); 
        toast({
          title: "Request Submitted!", // This could be localized too if dictionary is passed/fetched
                ...
                <SelectContent>
                  {availableServices.map(service => (
                    <SelectItem key={service.id} value={service.title}>
                      {service.title} {/* Service titles are not translated in this pass */}
                    </SelectItem>
                  ))}
