...
const statusColors = [
  ...
  "hsl(var(--chart-3))", // Quoted
  "hsl(var(--chart-4))"  // Closed
];
...
const inquiriesTimeChartConfig = {
  count: { label: "Inquiries", color: "hsl(var(--primary))", },
} satisfies ChartConfig;
...
async function getDashboardData(): Promise<DashboardStats> {
  ...
  const servicesArray: Service[] = servicesSnapshot.exists() ? Object.keys(servicesData).map(key => ({id: key, ...servicesData[key]})) : [];
  ...
  const testimonialsArray: Testimonial[] = testimonialsSnapshot.exists() ? Object.keys(testimonialsData).map(key => ({id: key, ...testimonialsData[key]})) : [];

  const projectsSnapshot = await get(child(dbRef, 'projects'));
  const projectsData = projectsSnapshot.exists() ? projectsSnapshot.val() : {};
  const projectsArray: Project[] = projectsSnapshot.exists() ? Object.keys(projectsData).map(key => ({id: key, ...projectsData[key]})) : [];
  ...
  const clientInquiries: ClientInquiry[] = inquiriesSnapshot.exists() ? Object.keys(inquiriesData).map(key => ({id:key, ...inquiriesData[key]})) : [];
  ...
  const serviceRequestChartData = Object.entries(serviceRequestCounts)
    .map(([name, count], index) => ({ name, count, fill: serviceSpecificColors[index % serviceSpecificColors.length] }))
  ...
  const inquiriesTimeChartData = Object.values(inquiriesByMonth)
    ...
    .map(item => ({ month: format(item.date, 'MMM yyyy'), count: item.count }));
...
export default function AdminDashboardPage() {
  ...
  const { user, loading } = useAuth(); // Usamos el hook de autenticación
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [user]); // Dependencia de user para recargar cuando cambie

  // Mostrar loader mientras se verifica autenticación o carga datos
  if (loading || isLoading || !dashboardData) {
    return (
  ...
  const stats = [
    { title: "Total Services", value: totalServices, icon: ShoppingBag, href: "/admin/services" },
    { title: "Total Testimonials", value: totalTestimonials, icon: Users, href: "/admin/testimonials" },
    { title: "Total Projects", value: totalProjects, icon: Briefcase, href: "/admin/projects" },
    { title: "New Inquiries", value: newInquiries, icon: MessageSquare, href: "/admin/inquiries?statusFilter=New" },
    { title: "Total Inquiries", value: totalInquiries, icon: MessageSquare, href: "/admin/inquiries" },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Resto del código de gráficos y secciones... */}
      <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-2">
          ...
          <CardContent>
            {serviceRequestChartData.length > 0 ? (
              <ChartContainer config={serviceRequestChartConfig} className="min-h-[250px] w-full">
                <BarChart accessibilityLayer data={serviceRequestChartData} layout="vertical"
                  margin={{left: 20, right: 20, top: 5, bottom: 5}} isAnimationActive={true} animationDuration={700}>
        ...
        {/* Gráfico de estado de consultas */}
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="text-xl text-primary flex items-center gap-2">
            ...
            {totalInquiries > 0 && statusChartData.some(s => s.count > 0) ? (
              <ChartContainer config={statusChartConfig} className="min-h-[250px] w-full max-w-[300px] aspect-square">
                <PieChart isAnimationActive={true} animationDuration={700}>
                  ...
                  <Pie data={statusChartData} dataKey="count" nameKey="status" innerRadius={60} outerRadius={90} paddingAngle={2}
                    isAnimationActive={true} labelLine={false}
                    label={({ cx, cy, midAngle, innerRadius, outerRadius, percent }) => {
                      ...
                      return percent > 0.05 ? (
                        <text x={x} y={y} fill="hsl(var(--foreground))" textAnchor={x > cx ? 'start' : 'end'}
                          dominantBaseline="central" className="text-xs">
      ...
      {/* Gráfico de consultas a lo largo del tiempo */}
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="text-xl text-primary flex items-center gap-2">
        ...
        <CardContent>
          {inquiriesTimeChartData.length > 0 ? (
            <ChartContainer config={inquiriesTimeChartConfig} className="min-h-[300px] w-full">
              <LineChart accessibilityLayer data={inquiriesTimeChartData}
                margin={{ left: 12, right: 20, top: 5, bottom: 5, }} isAnimationActive={true} animationDuration={700}>
                <CartesianGrid vertical={false} strokeDasharray="3 3"/>
                <XAxis dataKey="month" tickLine={false} axisLine={false} tickMargin={8} />
                <YAxis allowDecimals={false} />
                <ChartTooltip cursor={{strokeDasharray: '3 3', fill: 'hsl(var(--muted))'}}
                  content={<ChartTooltipContent indicator="line" />} />
                <Line dataKey="count" type="monotone" stroke="hsl(var(--primary))" strokeWidth={3}
